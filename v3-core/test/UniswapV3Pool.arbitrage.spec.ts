      ...
      for (const passiveLiquidity of [
        ...
        describe(`passive liquidity of ${formatTokenAmount(passiveLiquidity)}`, () => {
          const arbTestFixture = async ([wallet, arbitrageur]: Wallet[]) => {
            ...
            await fix.token0['transfer(address,uint256)'](arbitrageur.address, BigNumber.from(2).pow(254))
            await fix.token1['transfer(address,uint256)'](arbitrageur.address, BigNumber.from(2).pow(254))
