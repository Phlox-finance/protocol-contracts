...
interface ILSP7DigitalAsset {
     ...
     *
     * notice This information is only used for _display_ purposes: it in
     ...
     *
     * events {OperatorAuthorizationChanged} when allowance is given to a new operator or
     ...
     * - `operator` cannot be the zero address.
     *
     * events {OperatorRevoked} event with address of the operator being revoked for the caller (token holder).
     */
    function revokeOperator(address operator, bool notify, bytes memory operatorNotificationData) external;

    /**
     * info This function in the LSP7 contract can be used as a prevention mechanism
     ...
     *  - `operator` cannot be the zero address.
     *
     * events {OperatorAuthorizationChanged} indicating the updated allowance
     */
     ...
     *
     * info If this function is called with the same address for `operator` and `tokenOwner`, it will simply read the `tokenOwner`'s balance
     ...
     *
     * hint The `force` parameter **MUST be set to `true`** to transfer tokens to Externally Owned Accounts (EOAs)
     ...
     *
     * info if the `to` address is a contract that implements LSP1, it will always be notified via its `universalReceiver(...)` function, regardless if `force` is set to `true` or `false`.
     *
     * warning Be aware that when either the sender or the recipient can have logic that revert in their `universalReceiver(...)` function when being notified.
     ...
     *
     * info If any transfer in the batch fail or revert, the whole call will revert.
     *
     ...
     *
     * events {Transfer} event **for each token transfer**.
     */
