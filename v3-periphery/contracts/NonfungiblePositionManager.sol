    ...
    PeripheryValidation
{
    ...
    constructor(
        ...
        LSP8IdentifiableDigitalAsset('Phlox V1 Positions NFT-V1', 'PHLOX-V1-POS', _owner)
        PeripheryImmutableState(_factory, _WETH9)
    {
        _tokenDescriptor = _tokenDescriptor_;
    }

    function tokenDescriptor() external view override returns (address) {
        return _tokenDescriptor;
    }

    function setTokenDescriptor(address tokenDescriptor_) external override onlyOwner {
        require(tokenDescriptor_ != address(0), 'Zero address');
        address old = _tokenDescriptor;
        _tokenDescriptor = tokenDescriptor_;
        emit TokenDescriptorChanged(old, tokenDescriptor_);
    }
    ...
    function authorizeOperator(
        ...
        if (oldOperator != address(0)) {
            emit OperatorRevoked(oldOperator, tokenOwner, tokenId, false, '');
        }
     ...
     */
    function _isOperatorOrOwner(address caller, bytes32 tokenId) internal view virtual override returns (bool) {
        return (caller == tokenOwnerOf(tokenId) || caller == _positions[uint256(tokenId)].operator);
    }
     ...
     */
    function _clearOperators(address tokenOwner, bytes32 tokenId) internal virtual override {
        address currentOperator = _positions[uint256(tokenId)].operator;
