...
interface ILSP8IdentifiableDigitalAsset {
     ...
     * info if the `tokenId` is not owned by any address, the returned address will be `address(0)`
     */
     ...
     * events {TokenIdDataChanged} event for each pair.
     */
     ...
     *
     * events {OperatorAuthorizationChanged} event.
     */
     ...
     * - `operator` cannot be the zero address.
     *
     * events {OperatorRevoked} event with address of the operator being revoked for the caller (token owner)..
     */
     ...
     *
     * info The tokenOwner is its own operator.
     */
