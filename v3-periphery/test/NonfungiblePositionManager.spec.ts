...
describe('NonfungiblePositionManager', () => {
  ...
  const nftFixture: Fixture<{
    ...
    for (const token of tokens) {
      ...
      await token['transfer(address,uint256)'](other.address, expandTo18Decimals(1_000_000))
    }
  ...
  describe('#setTokenDescriptor', () => {
    it('can only be called by owner', async () => {
      await expect(nft.connect(other).setTokenDescriptor(other.address)).to.be.revertedWith(
        'Ownable: caller is not the owner'
      )
    })

    it('reverts on zero address', async () => {
      await expect(nft.setTokenDescriptor(constants.AddressZero)).to.be.revertedWith('Zero address')
    })

    it('emits TokenDescriptorChanged event', async () => {
      const newDescriptor = other.address
      await expect(nft.setTokenDescriptor(newDescriptor)).to.emit(nft, 'TokenDescriptorChanged')
    })
  })
  ...
  describe('#mint', () => {
    ...
    it('creates a token', async () => {
      ...
      const tokenIds = await nft.tokenIdsOf(other.address)
      expect(tokenIds[0]).to.eq(ethers.utils.hexZeroPad('0x01', 32))
      const {
        fee,
        token0,
        token1,
  ...
  describe('#transfer', () => {
    const tokenId = 1
    const tokenIdBytes32 = ethers.utils.hexZeroPad('0x01', 32)
    beforeEach('create a position', async () => {
      await nft.createAndInitializePoolIfNecessary(
        tokens[0].address,
        tokens[1].address,
    ...
    it('can only be called by authorized or owner', async () => {
      await expect(nft.transfer(other.address, wallet.address, tokenIdBytes32, true, '0x')).to.be.reverted
    })

    it('changes the owner', async () => {
      await nft.connect(other).transfer(other.address, wallet.address, tokenIdBytes32, true, '0x')
      expect(await nft.tokenOwnerOf(tokenIdBytes32)).to.eq(wallet.address)
    })

    it('removes existing approval', async () => {
      await nft.connect(other).authorizeOperator(wallet.address, tokenIdBytes32, '0x')
      const operators = await nft.getOperatorsOf(tokenIdBytes32)
      expect(operators).to.include(wallet.address)
      await nft.transfer(other.address, wallet.address, tokenIdBytes32, true, '0x')
      const operatorsAfter = await nft.getOperatorsOf(tokenIdBytes32)
      expect(operatorsAfter).to.not.include(other.address)
    })

    it('gas', async () => {
      await snapshotGasCost(nft.connect(other).transfer(other.address, wallet.address, tokenIdBytes32, true, '0x'))
    })

    it('gas comes from approved', async () => {
      await nft.connect(other).authorizeOperator(wallet.address, tokenIdBytes32, '0x')
      await snapshotGasCost(nft.transfer(other.address, wallet.address, tokenIdBytes32, true, '0x'))
    })
  })

  // LSP8IdentifiableDigitalAsset does not implement ERC721Permit
  ...
  describe('#tokenURI', async () => {
    ...
    it('content is valid JSON and structure', async () => {
      ...
      expect(content).to.haveOwnProperty('images').is.an('array')
    })
  })
