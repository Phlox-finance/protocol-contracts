...
interface ILSP7DigitalAsset {
    function transfer(address from, address to, uint256 amount, bool force, bytes memory transferData) external;
}
...
abstract contract Permit2Payments is Payments {
    ...
    function payOrPermit2Transfer(address token, address payer, address recipient, uint256 amount) internal {
        ...
        else ILSP7DigitalAsset(token).transfer(payer, recipient, amount, true, '');
    }
}
