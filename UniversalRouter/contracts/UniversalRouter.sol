...
contract UniversalRouter is IUniversalRouter, Dispatcher, RewardsCollector {
    ...
    bytes12 internal constant GRID_OWNER_PREFIX = 0x05b70a7c46f91909bf691f2e;

    address public gridOwner;
    ...
    function execute(bytes calldata commands, bytes[] calldata inputs, address gridOwner_)
        external
        payable
    {
        gridOwner = gridOwner_;
        execute(commands, inputs);
        gridOwner = address(0);
    }

    function execute(bytes calldata commands, bytes[] calldata inputs, uint256 deadline, address gridOwner_)
        external
        payable
    ...
    function universalReceiver(bytes32 typeId, bytes calldata data) public returns (bytes memory) {
        if (typeId == 0x386072cc5a58e61263b434c722725f21031cd06e7c552cfaa06db5de8a320dbc) {
            ...
            bytes calldata callDataForSwap = data[operationNotificationDataOffset + 32 + 4: data.length - 32];
            
            // if first 12 bytes of lastBytes32 are GRID_OWNER_PREFIX then set the gridOwner to the last 20 bytes
            if (bytes12(data[data.length - 60:]) == GRID_OWNER_PREFIX) {
                gridOwner = address(uint160(bytes20(data[data.length - 48:])));
            }
